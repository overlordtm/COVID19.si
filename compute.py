...
def parse_csv(filename):
  df = pd.read_csv(filename, parse_dates=['Date'], index_col=0, dayfirst=True, header=0, low_memory=False, dtype=float)
...
if __name__ == '__main__':
  ...
  tests_and_confirmed['Daily tests'] = tests_and_confirmed['Tests made'].diff()
  tests_and_confirmed['Daily confirmed cases'] = tests_and_confirmed['Confirmed cases'].diff()

  dfs = [tests_and_confirmed, by_situation, by_source, by_region, healthcare]

  totals = reduce(lambda left,right: pd.merge(left,right, left_index=True, right_index=True, how='outer'), dfs)
  print(totals)
